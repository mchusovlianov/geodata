...
func StartContainer(image string, port string, health string, args ...string) (*Container, error) {
	arg := []string{"run", "-P", "-d"}

	if health != "" {
		arg = append(arg, "--health-cmd", health)
	}
...
func Status(id string) (string, error) {
	tmpl := fmt.Sprintf("{{.State.Health.Status}}")

	cmd := exec.Command("docker", "inspect", "-f", tmpl, id)
	var out bytes.Buffer
	cmd.Stdout = &out
	if err := cmd.Run(); err != nil {
		return "", fmt.Errorf("could not get container's status %s: %w", id, err)
	}
